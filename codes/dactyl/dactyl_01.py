    ...
    def __init__(self, *args, **kwargs):
        super(CustomACLstmPolicy, self).__init__(
            ...
            **kwargs,
            ...
            net_arch=dict(pi=[256, 256], vf=[256, 256]),  # Changed from list to dict
...
class RewardCallback(BaseCallback):
    """
    Custom callback for logging average rewards over every `avg_interval` episodes,
    episode numbers, iteration numbers, and logging to Weights & Biases (W&B).
    """
    ...
    def __init__(self, avg_interval=100):
        ...
        self.median_rewards = {}
        self.avg_rewards = {}
        self.iteration_num = 0  # Initialize iteration counter
                ...
                if done:
                    ...
                    if len(self.rewards_list) == self.avg_interval:
                        ...
                        wandb.log({
                            ...
                            "Episodes": block_num * self.avg_interval,
    ...
    def _on_training_end(self) -> None:
        ...
        if len(self.rewards_list) > 0:
            ...
            # Log the final set of rewards to W&B
            wandb.log({
                "Iteration": self.iteration_num,
...
def train_on_gpu(gpu_id: int, num_envs: int, total_timesteps: int, num_steps: int, save_interval: int):
    ...
    model = RecurrentPPO(
        ...
        tensorboard_log=f"/home/dgusain/Bot_hand/dactyl/logs/01/gpu{gpu_id}"  # Enable TensorBoard logging
    )
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_path=f"/home/dgusain/Bot_hand/dactyl/checkpoints/01/checkpoints_gpu{gpu_id}", 
    ...
    eval_callback = EvalCallback(
        eval_env,
        best_model_save_path=f"/home/dgusain/Bot_hand/dactyl/01/best_models_gpu{gpu_id}",
        log_path=f"/home/dgusain/Bot_hand/dactyl/01/eval_logs_gpu{gpu_id}",
    ...
    custom_wandb_callback = CustomWandbCallback(
        ...
        model_save_path=f"/home/dgusain/Bot_hand/dactyl/01/wandb_models_gpu{gpu_id}/",
        verbose=1
    )
    ...
    model.save(f"/home/dgusain/Bot_hand/dactyl/agents/final/01/agent_gpu{gpu_id}")
...
def main():
    num_gpus = 1
    num_envs_per_gpu = 6  # Adjust based on GPU memory and environment complexity
    n_iter = 15000
