...
class HandEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        total_rew += reward
        avg_rew = total_rew/self.steps_taken
        self.steps_taken += 1

        truncated = False
        info = {}

        return state.numpy(), avg_rew, done, truncated, info
...
class RewardCallback(BaseCallback):
    ...
    def plot_rewards(self):
        ...
        plt.grid()
        #plt.show()
        plt.savefig('/home/dgusain/Bot_hand/figs/fig_train_discrete6.png')  
...
def train_on_gpu(gpu_id: int, num_envs: int, episodes: int, steps_per_episode: int):
    ...
    model.save(f"/home/dgusain/Bot_hand/agents/agent_discrete6")
    return callback
