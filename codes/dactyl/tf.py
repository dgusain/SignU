...
class HandEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        self.set_joint_positions(joint_positions_dict, steps=100, delay=0.01)
...
class RewardCallback(BaseCallback):
    ...
    def plot_rewards(self):
        ...
        plt.grid()
        #plt.show()
        plt.savefig('/home/dgusain/Bot_hand/figs/fig_train_discrete5.png')  
...
def inference(gpu_id: int, num_envs: int, episodes: int, steps_per_episode: int):
    ...
    model_path = "/home/dgusain/Bot_hand/agents/agent_discrete5"
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(eval_rewards, label="Reward per Episode")
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.title("Reward vs Episode")
    plt.legend()
    plt.grid()
    plt.show()
    plt.savefig('/home/dgusain/Bot_hand/codes/dactyl/fig_predict_tf.png')  
...
def train_on_gpu(gpu_id: int, num_envs: int, episodes: int, steps_per_episode: int):
    ...
    model.save(f"/home/dgusain/Bot_hand/agents/agent_discrete5")
    return callback

def main():
    gpu_id = 3
    num_envs = 1     # Use a single environment for simpler episode count
    ...
    # inference
    inference(gpu_id, num_envs,episodes,steps_per_episode)
