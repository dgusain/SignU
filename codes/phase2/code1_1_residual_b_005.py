...
class HandEnv(gym.Env):
    def __init__(self):
        ...
        finger_weights = [
            1.0,  # ID 0: WristJoint1
            1.0,  # ID 1: WristJoint0
            1.5,  # ID 2: ForeFingerJoint3 (MCP)
            3.0,  # ID 3: ForeFingerJoint2 (MCP proximal)
            3.0,  # ID 4: ForeFingerJoint1 (PIP)
            3.0,  # ID 5: ForeFingerJoint0 (DIP)
            1.5,  # ID 6: MiddleFingerJoint3 (MCP)
            3.0,  # ID 7: MiddleFingerJoint2 (MCP proximal)
            3.0,  # ID 8: MiddleFingerJoint1 (PIP)
            3.0,  # ID 9: MiddleFingerJoint0 (DIP)
            1.0,  # ID 10: RingFingerJoint3 (MCP)
            2.0,  # ID 11: RingFingerJoint2 (MCP proximal)
            2.0,  # ID 12: RingFingerJoint1 (PIP)
            2.0,  # ID 13: RingFingerJoint0 (DIP)
            1.0,  # ID 14: LittleFingerJoint4 (CMC Joint rotation)
            2.0,  # ID 15: LittleFingerJoint3 (MCP)
            2.0,  # ID 16: LittleFingerJoint2 (MCP proximal)
            2.0,  # ID 17: LittleFingerJoint1 (PIP)
            2.0,  # ID 18: LittleFingerJoint0 (DIP)
            3.0,  # ID 19: ThumbJoint4 (CMC Joint abduction/adduction)
            3.0,  # ID 20: ThumbJoint3 (CMC Joint flexion/extension)
            3.0,  # ID 21: ThumbJoint2 (CMC Joint rotation)
            3.0,  # ID 22: ThumbJoint1 (MCP)
            3.0,  # ID 23: ThumbJoint0 (IP)
            0.0   # ID 24: None
        ]
        # Converting joint weights to a tensor
        self.joint_weights = torch.tensor(joint_weights, dtype=torch.float32)
        self.finger_weights = torch.tensor(finger_weights, dtype=torch.float32)
        # Normalize the weights so that their sum equals 1
        self.joint_weights /= self.joint_weights.sum()
        self.finger_weights /= self.finger_weights.sum()
        self.finger_coeff = 0.05
    ...
    def calculate_confidence(self, state: torch.Tensor) -> torch.Tensor:
        ...
        weighted_similarity = similarity * self.joint_weights * ((self.finger_weights)**(self.finger_coeff))
...
def train_on_gpu(gpu_id: int, num_envs: int, total_timesteps: int, num_steps: int, save_interval: int):
    ...
    model = RecurrentPPO(
        ...
        verbose=1,
        device=device,
        ent_coef=0.1,              # Adjusted entropy coefficient
    ...
    model.save(f"/home/easgrad/dgusain/Bot_hand/agents/agent_code1_1_residual_b_005_gpu{gpu_id}")
...
def main():
    ...
    plt.grid(True)
    plt.savefig('/home/easgrad/dgusain/Bot_hand/figs/fig_code1_1_residual_b_005.png')  
    plt.close() 
    print("Plot saved as fig_code1_1_residual_b_005.png")
