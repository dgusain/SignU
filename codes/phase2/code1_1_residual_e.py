...
class HandEnv(gym.Env):
    ...
    def calculate_confidence(self, state: torch.Tensor) -> torch.Tensor:
        ...
        # Scale angular displacement from [0, 2π] to [0, 100]
        ...
        # Aggregate weighted angular displacements to compute average confidence
        # Since higher displacement indicates lower confidence, invert the scaled displacement
        angular_disp_score = 100 - weighted_angular_disp.sum()  # Scalar, 0-100
        
        return angular_disp_score
...
def train_on_gpu(gpu_id: int, num_envs: int, total_timesteps: int, num_steps: int, save_interval: int):
    ...
    model = RecurrentPPO(
        ...
        verbose=1,
        device=device,
        ent_coef=0.2,              # Adjusted entropy coefficient
    ...
    model.save(f"/home/easgrad/dgusain/Bot_hand/agents/agent_code1_1_residual_e_gpu{gpu_id}")
...
def main():
    ...
    plt.grid(True)
    plt.savefig('/home/easgrad/dgusain/Bot_hand/figs/fig_code1_1_residual_e.png')  
    plt.close() 
    print("Plot saved as fig_code1_1_residual_e.png")
