...
class CustomFE(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.Space, features_dim: int = 256):
        ...
        self.net = nn.Sequential(
            nn.Linear(observation_space.shape[0], 128),  # input:24, first layer: 128
            nn.ReLU(),
            nn.LayerNorm(128),
            nn.Linear(128, 256),
            nn.ReLU(),
...
def train_on_gpu(gpu_id: int, num_envs: int, total_timesteps: int, num_steps: int, save_interval: int):
    ...
    model = RecurrentPPO(
        ...
        verbose=1,
        device=device,
        ent_coef=0.05,              # Adjusted entropy coefficient
    ...
    model.save(f"/home/easgrad/dgusain/Bot_hand/agents/agent_code1_1_basic_gpu{gpu_id}")
...
def main():
    ...
    plt.grid(True)
    plt.savefig('/home/easgrad/dgusain/Bot_hand/figs/fig_code1_1_basic.png')  
    plt.close() 
    print("Plot saved as fig_code1_1_basic.png")
