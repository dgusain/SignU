...
class HandEnv(gym.Env):
    ...
    def calculate_done(self, state: torch.Tensor, flag: bool) -> bool:
        if flag:
            return False
        confidence, ang_disp = self.calculate_metrics(state)
        return (confidence >= self.target_threshold) or (ang_disp <= self.ang_threshold)

    def calculate_reward(self, state: torch.Tensor, flag: bool) -> torch.Tensor:
        confidence, angular_displacement = self.calculate_metrics(state)
        conf_wt, ad_wt = 1.0, -0.1  # angular disp has weight of 10%
        displacement_penalty = ad_wt * angular_displacement
        progress_reward = confidence * conf_wt - 50
        if flag:
            
            return progress_reward + displacement_penalty  # Reward calculation
        else:
            if confidence > 85:
                return progress_reward/2.0 + displacement_penalty # encouraging the model in the right direction
            else:
                return progress_reward/4.0 + displacement_penalty # lesser reward

    def calculate_metrics(self, state: torch.Tensor) -> [torch.Tensor, torch.Tensor]:
        ...
        dot_product = torch.abs(torch.sum(rendered_quat * gt_quat, dim=1))
        ...
        angle_deg = torch.where(angle_deg > 180.0, 360.0 - angle_deg, angle_deg)
        angular_displacement = (angle_deg/180.0) * 100
        avg_angular_disp = angular_displacement.mean()

        return avg_confidence, avg_angular_disp
...
def train_on_gpu(gpu_id: int, num_envs: int, total_timesteps: int, num_steps: int, save_interval: int):
    ...
    model.save(f"/home/easgrad/dgusain/Bot_hand/agents/agent_code2_1_gpu{gpu_id}")
...
def main():
    ...
    plt.grid(True)
    plt.savefig('/home/easgrad/dgusain/Bot_hand/figs/fig_code2_1.png')  
    plt.close() 
    print("Plot saved as fig_code2_1.png")
