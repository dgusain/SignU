...
class RewardCallback(BaseCallback):
    """
    Custom callback for logging rewards, episode numbers, and iteration numbers.
    Also stores episode rewards for plotting.
    """
...
def train_on_gpu(gpu_id: int, num_envs: int, total_timesteps: int, num_steps: int, save_interval: int):
    ...
    model.save(f"/home/easgrad/dgusain/Bot_hand/agents/agent_g3b_{gpu_id}_weighted")

    return callback  # Return the callback to access episode_rewards

def main():
    gpu_id = 0          # Set your GPU ID here
    num_envs = 6        # Number of parallel environments (adjust based on your GPU memory)
    n_iter = 2000         # Number of iterations you want to run
    ...
    # Extract episode numbers and rewards from the callback
    episode_numbers = list(callback.median_rewards.keys())
    rewards = list(callback.median_rewards.values())

    # Plotting the rewards vs episodes
    plt.figure(figsize=(10, 6))
    plt.plot(episode_numbers, rewards, marker='o', linestyle='-', color='b')
    plt.xlabel('Episode Number')
    plt.ylabel('Reward')
    plt.title('Reward vs Episode Number')
    plt.grid(True)
    plt.savefig('/home/easgrad/dgusain/Bot_hand/figs/fig_g3b_weighted.png')  # Save the figure
    plt.close()  # Close the figure to free memory
    print("Plot saved as fig_g3b_weighted.png")
