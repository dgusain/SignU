...
class HandEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        if done:
            ...
            reward = -0.5  # Penalize extra steps
    ...
    def calculate_reward(self, state: torch.Tensor, flag: bool) -> torch.Tensor:
        if flag:
            ...
            return (confidence -50)/2.0  # Intermediate steps have a fixed penalty
...
def train_on_gpu(gpu_id: int, num_envs: int, total_timesteps: int, num_steps: int, save_interval: int):
    ...
    model.save(f"/home/easgrad/dgusain/Bot_hand/agents/agent_g3c_{gpu_id}_weighted")

    return callback  # Return the callback to access episode_rewards

def main():
    gpu_id = 5          # Set your GPU ID here
    num_envs = 6        # Number of parallel environments (adjust based on your GPU memory)
    n_iter = 5000         # Number of iterations you want to run
    ...
    plt.grid(True)
    plt.savefig('/home/easgrad/dgusain/Bot_hand/figs/fig_g3c_weighted.png')  # Save the figure
    plt.close()  # Close the figure to free memory
    print("Plot saved as fig_g3c_weighted.png")
