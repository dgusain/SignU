...
class HandEnv(gym.Env):
    def __init__(self):
        ...
        with open("quat_v.pkl",'rb') as file:
    ...
    def calculate_reward(self, difference_angle: float) -> float:
        reward = max(0.0, 5.0 - difference_angle * 5)  # Scales from 5.0 to 0.0
        #reward -= 0.0001 * self.steps_taken
        return reward
...
class RewardCallback(BaseCallback):
    def __init__(self, num_envs, plot_freq=100, save_path='/home/dgusain/Bot_hand/figs/05_letV/', verbose=0):
...
def inference(gpu_id, num_envs):
    ...
    model_path = "/home/dgusain/Bot_hand/agents/agent_tf_05_letV"
...
def train_on_gpu(gpu_id: int, num_envs: int, episodes: int, steps_per_episode: int):
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000,  
        save_path="/home/dgusain/Bot_hand/agents/checkpoints/05_V/",  
        name_prefix="agent_tf"  
    )
    plot_frequency = 100 
    save_plots_path = "/home/dgusain/Bot_hand/figs/05_letV/"  
    ...
    model.save(f"/home/dgusain/Bot_hand/agents/agent_tf5_letV")
...
def main():
    gpu_id = 1
    num_envs = min(32, cpu_count())  # Use up to 8 or available CPU cores
