...
class HandEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.sim.forward()
        self.steps_taken = 0
        obs = self.sim.data.qpos[:24].astype(np.float32).copy()
    ...
    def step(self, action: np.ndarray):
        ...
        if new_difference_angle < self.target_threshold:
            ...
            self.ep += 1
        elif 0.5 < new_difference_angle < 1:
            reward = 2.0
            done = False
        elif 0.1 < new_difference_angle <= 0.5:
            reward = 3.0
            done = False
        elif self.target_threshold < new_difference_angle <= 0.1:
            reward = 4.0
            done = False
...
class RewardCallback(BaseCallback):
    def __init__(self, num_envs, plot_freq=100, save_path='/home/dgusain/Bot_hand/figs/04/', verbose=0):
...
def inference(gpu_id, num_envs):
    ...
    model_path = "/home/dgusain/Bot_hand/agents/agent_tf_04"
...
def train_on_gpu(gpu_id: int, num_envs: int, episodes: int, steps_per_episode: int):
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=50000,  
        save_path="/home/dgusain/Bot_hand/agents/checkpoints/04/",  
        name_prefix="agent_tf"  
    )
    plot_frequency = 100 
    save_plots_path = "/home/dgusain/Bot_hand/figs/04/"  
    ...
    model.save(f"/home/dgusain/Bot_hand/agents/agent_tf4")
...
def main():
    gpu_id = 1
    num_envs = min(8, cpu_count())  # Use up to 8 or available CPU cores
    episodes = 10000
