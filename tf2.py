    ...
    def __init__(self, *args, **kwargs):
        super(CustomACLstmPolicy, self).__init__(
            ...
            **kwargs,
            ...
            net_arch=dict(pi=[1024, 1024], vf=[1024, 1024]),  # Changed from list to dict
...
class RewardCallback(BaseCallback):
    """
    Reward Callback that handles multiple environments by tracking rewards individually.
    """
        ...
        for i, (done, reward) in enumerate(zip(dones, rewards)):
            ...
            if done:
                ...
                if len(self.episode_rewards[i]) > 0:
                    ...
                    avg_reward = 0.0
                    max_reward = 0.0
                print(f"Episode {self.episode_counts[i]} | Env {i+1} | Last Reward: {reward} | Avg. Reward: {avg_reward} | Max Reward: {max_reward}")
    ...
    def plot_rewards(self):
        ...
        plt.legend()
        plt.grid()
        plt.savefig('/home/dgusain/Bot_hand/figs/fig_train_tf2.png')  
...
def train_on_gpu(gpu_id: int, num_envs: int, episodes: int, steps_per_episode: int):
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_path="/home/dgusain/Bot_hand/agents/checkpoints/03/",  # Directory for saving models
        name_prefix="agent_tf"  # Prefix for saved model filenames
    )
    reward_callback = RewardCallback(num_envs=num_envs)
