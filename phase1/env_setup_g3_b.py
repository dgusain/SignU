...
class RewardCallback(BaseCallback):
    ...
    def __init__(self):
        ...
        self.episode_rewards = {}  # Dictionary to store episode number and rewards
        ...
        if np.any(dones):
            for done, reward in zip(dones, rewards):
                if done:
                    ...
                    self.episode_rewards[self.episode_num] = reward  # Store in dict
                    print(f"Iteration: {self.iteration_num} | Episode: {self.episode_num} | Reward: {reward:.2f} | Max Reward: {self.max_reward:.2f}")
        return True
...
def main():
    ...
    episode_numbers = list(callback.episode_rewards.keys())
    rewards = list(callback.episode_rewards.values())
