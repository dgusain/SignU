...
class HandEnv(gym.Env):
    ...
    def step(self, action):
        ...
        #print(f"Action: {rescaled_action} | Min: {self.actuator_min} | Max: {self.actuator_max}")
...
class RewardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals['dones'][0]:
            ...
            print(f"Episode: {self.episode_num} | Reward: {reward} | Max_Reward: {self.max_reward}")
        return True



def train_on_gpu(gpu_id, iterations_per_gpu, save_interval=25):
    os.environ['CUDA_VISIBLE_DEVICES'] = str(gpu_id)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"GPU {gpu_id}: Using {device} device")

    env = HandEnv()
    model = RecurrentPPO('MlpLstmPolicy', env, verbose=1, device=device, ent_coef=0.05, 
                     learning_rate=0.0001, clip_range=0.3, n_steps=2056)
    ...
    for iteration in range(iterations_per_gpu):
        ...
        if (iteration + 1)% save_interval == 0:
            model.save(f"hand_pose_ppo_gpu_{gpu_id}_iter_{iteration}")
...
def main():
    total_iterations = 20  
    train_on_gpu(0, total_iterations)
