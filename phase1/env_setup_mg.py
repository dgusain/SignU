# Reward calculated for each step. Also no penalization for extra steps taken.
...
def train_on_gpu(gpu_id, iterations_per_gpu):
   ...
   model = PPO(
    ...
    verbose=1, 
    device=device, 
    ent_coef=0.01,  # Encourage exploration by increasing entropy
    learning_rate=0.0001,  # Smaller learning rate to explore more
    clip_range=0.3,  # Increase clipping range
    ...
    for iteration in range(iterations_per_gpu):
        ...
        obs, _ = env.reset()
        total_reward = 0
        for _ in range(100):  # Specify the number of steps for evaluation
