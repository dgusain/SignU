...
class RewardCallback(BaseCallback):
        ...
        if np.any(dones):
            for done, reward in zip(dones, rewards):
                if done:
                    ...
                    if self.count_rewards == self.avg_interval:
                        avg_reward = self.sum_rewards / self.avg_interval
                        block_num = self.episode_num // self.avg_interval
                        self.avg_rewards[block_num] = avg_reward
                        print(f"Iteration: {self.iteration_num} | Episodes: {block_num * self.avg_interval} | Avg Reward: {avg_reward:.2f} | Max Reward: {self.max_reward:.2f}")
                        # Reset sum and count
                        self.sum_rewards = 0.0
                        self.count_rewards = 0
        return True
    ...
    def _on_training_end(self) -> None:
        ...
        """
        if self.count_rewards > 0:
            avg_reward = self.sum_rewards / self.count_rewards
            block_num = self.episode_num // self.avg_interval + 1
            self.avg_rewards[block_num] = avg_reward
            print(f"Training End | Episodes: {self.episode_num} | Avg Reward: {avg_reward:.2f} | Max Reward: {self.max_reward:.2f}")
...
def main():
    ...
    n_iter = 20         # Number of iterations you want to run
    ...
    block_numbers = list(callback.avg_rewards.keys())
    avg_rewards = list(callback.avg_rewards.values())
    ...
    plt.plot(episode_numbers, avg_rewards, marker='o', linestyle='-', color='b')
    plt.xlabel('Episode Number')
    plt.ylabel('Average Reward (per 100 episodes)')
    plt.title('Average Reward vs Episode Number')
    plt.grid(True)
    plt.savefig('g3_fig.png')  # Save the figure
    plt.close()  # Close the figure to free memory
    print("Plot saved as g3_fig.png")
